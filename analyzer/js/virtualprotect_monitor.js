...
if (vpAddress) {
    ...
    Interceptor.attach(vpAddress, {
        onEnter: function(args) {
            // HACK: Use a flag to avoid printing recursive calls from Frida's own operations
            ...
            if (protection & 0xf0) {
                const message = {
                    ...
                    // ⭐ NEW: Capture the return address to see WHO called VirtualProtect
                ...
                if (address.compare(ptr("0x404000")) >= 0 && address.compare(ptr("0x2000000")) < 0) {
                    ...
                    // ⭐ NEW: Read the memory region now that it's being made executable
                    const dump = ptr(address).readByteArray(size);

                    // ⭐ NEW: Send the metadata AND the binary dump back to Python
                    send(message, dump);
                } else {
