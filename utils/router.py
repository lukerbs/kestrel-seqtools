...
class CommandRouter:
    ...
    def handle_command(self, command: str, sock: socket.socket):
        ...
        # If in NORMAL mode, execute as shell command
            ...
            self.current_dir = execute_command_stream(command, sock, self.current_dir)
        
        # If in another mode, warn
        else:
            msg = f"[WARNING: In {self.mode_manager.current_mode.value} mode. Use /stop to exit or /quit to uninstall.]\n"
            send_text(sock, msg)
...
Current mode: {mode}
"""
        send_text(sock, help_text.format(mode=mode_manager.current_mode.value))
